...
<%@ page
	import="util.*, java.sql.ResultSetMetaData, java.sql.ResultSet, java.sql.SQLException"%>
...
<script type="text/javascript">
	...
	<%
			...
			String sql;
			String sql2;
			sql = "select recurso.NumeroRecurso, NomeUtilizador, idAdmin, Tipo, Titulo, Resumo, DataHoraCarr, Bloquear, FaixaEtar, filme.tipoAut, pessoa.Nome from recurso, filme, pessoa where recurso.NumeroRecurso = filme.NumeroRecurso and filme.ccPessoa = pessoa.ccPessoa order by Titulo;";
			...
			while(result.next()){
				String[] arrayNormal = new String[11];
			...
			if(arrayFinal.get(ValorCerto)[7].equals("1")){
				...
				if(!existe){
					%>
					<h3>Nao existem comentarios neste filme</h3>
					<% 
				}
				...
				if(!existeCla){
					%>
					<h3>Nao existem classificacoes neste filme</h3>
					<% 
				}
...
function LoadVideo(){
	document.getElementById("divVideo").innerHTML=
		'<video id="videos" autoplay loop controls preload="auto" heigth="300" width="400" >'+
		'<source id="videoSource" src="ViewBlob?dsg=<%=recursoID%>">'+
		'Your browser sucks'+
		'</video>';
...
function insertComent(){
 ...
 if(comment == null || comment === ""){
	 alert(" Nao tem nenhum texto no seu comentario ");
 }
...
function insertClass(){
	 ...
	 var nota = document.getElementById("class").value;
	 
	 if(nota === "null"){
		 alert(" Tem de selecionar uma nota ");
	 }
