...
<body>
	<%
			...
			String sql;
			sql = "select recurso.NumeroRecurso, NomeUtilizador, idAdmin, Tipo, Titulo, Resumo, DataHoraCarr, Bloquear, FaixaEtar, fotografia.tipoAut, pessoa.Nome from recurso, fotografia, pessoa where recurso.NumeroRecurso = fotografia.NumeroRecurso and fotografia.ccPessoa = pessoa.ccPessoa order by Titulo;";
			...
			if(arrayFinal.get(ValorCerto)[7].equals("1")){
				...
				if(!existe){
					%>
					<h3>Nao existem comentarios nesta fotografia</h3>
					<% 
				}
				...
				if(!existeCla){
					%>
					<h3>Nao existem classificacoes nesta fotografia</h3>
					<% 
				}
...
function LoadFotografia(){
	...
	document.getElementById("divFoto").innerHTML=
		'<img id="img" heigth="300" width="400" src="ViewBlob?dsg=<%=recursoID%>">'
		'</img>';
}
