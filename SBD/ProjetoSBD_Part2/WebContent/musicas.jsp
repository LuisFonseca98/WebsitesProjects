...
<%
			...
			String sql;
			sql = "select recurso.NumeroRecurso, NomeUtilizador, idAdmin, Tipo, Titulo, Resumo, DataHoraCarr, Bloquear, FaixaEtar, musica.tipoAut, pessoa.Nome from recurso, musica, pessoa where recurso.NumeroRecurso = musica.NumeroRecurso and musica.ccPessoa = pessoa.ccPessoa order by Titulo;";
			...
			if(arrayFinal.get(ValorCerto)[7].equals("1")){
				...
				if(!existe){
					%>
					<h3>Nao existem comentarios nesta musica</h3>
					<% 
				}
				...
				if(!existeCla){
					%>
					<h3>Nao existem classificacoes nesta musica</h3>
					<% 
				}
...
function LoadAudio(){
	document.getElementById("divAudio").innerHTML=
		'<audio id="musicas" autoplay loop controls >'+
		'<source id="audioSource" src="ViewBlob?dsg=<%=recursoID%>">'+
		'Your browser sucks'+
		'</audio>';
	document.getElementById("musicas").play();
}
