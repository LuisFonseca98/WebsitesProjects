...
class ColorHistogram {
    ...
    count_Pixels (pixels) {
        ...
        let npixels = pixels.data.length;
        for (let i = 0; i < npixels; i += 4) {
            ...
            for (let j = 0; j < this.redColor.length; j++) {
                let d1 = Math.abs(r - this.redColor[j]);
                let d2 = Math.abs(g - this.greenColor[j]);
                let d3 = Math.abs(b - this.blueColor[j]);
                let dm = d1 + d2 + d3;

                if (d1 < 70 && d2 < 70 && d3 < 70 && dm < 160) {
                    hist[j]++;
                }
    ...
    moments (imgobj, cnv) {
        ...
        for(let i = 0; i < this.h_block; i++){
            for(let j = 0; j < this.v_block; j++){
                ...
                let h = 0, s = 0, v = 0;
                let H = [], S = [], V = [];
                let h_contador = 0, s_contador = 0, v_contador = 0;
                ...
                h_contador = 0; s_contador = 0; v_contador = 0;
